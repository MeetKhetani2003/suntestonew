...
export function ProductCard({ product, onAdd, index = 0 }: { product: Product; onAdd: (product: Product) => void; index?: number }) {
  ...
  return (
    <article className="product-card reveal" style={{ transitionDelay: `${index * 60}ms`, display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div className="product-card-art" style={{ background: cardBg, overflow: 'hidden', height: '480px', position: 'relative' }}>
        <Link href={`/products/${product.slug}`} className="product-art-link" aria-label={`View ${product.name}`}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', position: 'relative', overflow: 'hidden', transform: 'scale(1)', transition: 'transform 0.6s cubic-bezier(0.2, 0.8, 0.2, 1)' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'}>
            <img src={product.image} alt={product.name} style={{ width: '85%', height: '85%', objectFit: 'contain', filter: 'drop-shadow(0 20px 40px rgba(0,0,0,0.15))' }} />
          </div>
        </Link>
        {product.badge && <span className="product-badge" style={{ position: 'absolute', background: '#000', color: '#fff', fontSize: '11px', fontWeight: 600, padding: '8px 16px', top: '24px', left: '24px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{product.badge}</span>}
      </div>
